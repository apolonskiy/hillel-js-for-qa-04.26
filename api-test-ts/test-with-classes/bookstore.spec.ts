import { Account, BookStore, TCredentials } from "../api";
import { faker } from "@faker-js/faker";

let userToken: string;

let userCreds: TCredentials;
...
describe("First api tests for Bookstore - Classes based", () => {
  ...
  test("Post user book - success scenario", async () => {
    ...
    const resp = await bookStore.postBooks(
      createdUserId,
      getBooksResp.data.books.map((b: { isbn: string }) => b.isbn),
      userToken,
    );
    expect(resp.status).toBe(201);
    expect(resp.data.books).toEqual(
      getBooksResp.data.books.map((b: { isbn: string }) => ({ isbn: b.isbn })),
    );
  });
});
